...
#ifdef LB_AQUA
          u32 *as_iterator;
          f32 score0 = (vip_shm->msg_out_cache->body[asindex0].n_flow_on + 1)/(1e-9 + vip_shm->msg_in_cache->score[asindex0]), score1;
          vec_foreach(as_iterator, vip0->as_indexes)
          {
            score1 = (vip_shm->msg_out_cache->body[*as_iterator].n_flow_on + 1)/(1e-9 + vip_shm->msg_in_cache->score[*as_iterator]);
            if (score1 < score0)
            {
              asindex0 = *as_iterator;
              score0 = score1;
            }
          }
#endif // LB_AQUA
