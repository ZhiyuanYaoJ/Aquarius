...
#define PT_OFFSET 3 // offset to calibrate server processing time with TCP timestamp options
...
#define if_inconsistent(bucket, index, buffer) \
        if (PREDICT_FALSE( \
        (bucket->tcp_flag[index] != TCP_FLAGS_NULL) \
        && ((buffer->src_ip != bucket->src_ip[index]) || (buffer->src_port != bucket->src_port[index])) \
        ))
#define register_new_flow(sym) \
        ({ \
        bucket->t_init[index] = buffer->time_now; \
        bucket->src_ip[index] = buffer->src_ip; \
        bucket->src_port[index] = buffer->src_port; \
        stat->n_flow++; \
        ref_as->t_last_flow = buffer->time_now; \
        ref_lb->t_last_flow = buffer->time_now; \
        buffer->d_n_flow += 1; \
        packet_type = SPT_##sym; \
        })
#define register_reservoir_as(ptr, sym, key, value) \
        ptr->sym[key] = value;
#define get_process_time(type, res_id) \
        ({ \
        int32_t pt = (int32_t) (tsecr - (u32) (bucket->t_last[index] * 1000) + ref_as->t0_ecr); \
        if (PREDICT_FALSE(pt < 0)) \
        { \
                ref_as->t0_ecr -= (u32) pt;\
                pt = 0; \
        } \
        tv_buffer_u.v = (u32) pt; \
        register_reservoir_as(reservoir_as, pt##type, res_id, tv_buffer_u); \
        })
...
ref_as_t * shm_get_as_ref(lb_vip_shm_t *lbshm, const u32 id);
