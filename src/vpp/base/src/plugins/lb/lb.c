...
next:
...
#ifdef LB_STATS
    shm_as_clear_cache(&vip->shm, *ip);
    SetBit (vip->shm.msg_out_cache->b_header, *ip); /* make cell set */
    shm_get_as_stat(&vip->shm, *ip)->as_index = *ip;
#endif // LB_STATS
  }
  vec_free(to_be_updated);
  ...
  vec_foreach(ip, to_be_added) {
    ...
    *as_index = as - lbm->ass;
#ifdef LB_STATS
    shm_as_clear_cache(&vip->shm, *as_index);
    SetBit (vip->shm.msg_out_cache->b_header, *as_index); /* make cell set */
    shm_get_as_stat(&vip->shm, *as_index)->as_index = *as_index;
#endif /* LB_STATS */
...
#ifdef LB_STATS
  shm_vip_init_mem (&vip->shm, *vip_index); // Initialize shared memory
#endif // LB_STATS
...
#ifdef LB_STATS
  /**
   * Remove opened shared memory and unlink
   */
  shm_vip_del_mem (&vip->shm);
#endif // LB_STATS
