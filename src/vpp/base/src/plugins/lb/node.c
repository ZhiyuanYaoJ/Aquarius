...
#ifdef LB_STATS
/**
* Parse TCP tsecr in header options
*
* @param th tcp header
* @return tsecr value (U32_MAX if no timestamp options)
*/
static_always_inline u32
get_tsecr(tcp_header_t * th)
{
  u8 *data = (u8 *)((char *)th + sizeof(tcp_header_t));
  u8 len = tcp_header_bytes(th) - (u8) sizeof(tcp_header_t);
  u32 tsecr = U32_MAX;
  u8 opt_len, kind;
  for (; len > 0; len -= opt_len, data += opt_len)
  {
    kind = data[0];
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
        {
...
#ifdef LB_STATS
              th0 = ip4_next_header (ip40); /* for stateful stats collection */
              /* just to get entropy -> no need to clib_net_to_host_u32 */ 
              sb->src_ip = ip40->src_address.as_u32;
#endif /* LB_STATS */            
            }
          else
            {
              ...
              len0 = clib_net_to_host_u16 (ip60->payload_length)
                  + sizeof(ip6_header_t);
#ifdef LB_STATS
              th0 = ip6_next_header (ip60); /* for stateful stats collection */
              /* just to get entropy -> no need to clib_net_to_host_u32 */ 
              sb->src_ip = ip60->src_address.as_u32[0] ^ ip60->src_address.as_u32[3]; /* for now take as_u32[0]^as_u32[3] to check if ip_src is consistent */
#endif /* LB_STATS */
            }

#ifdef LB_STATS
          /* Store info in buffer which will be forwarded to lb_hash functions */
          sb->time_now = (f32) vlib_time_now (vm);
          sb->tsecr = (u32) get_tsecr (th0);
          sb->tcp_ack = clib_net_to_host_u32 (th0->ack_number);
          sb->tcp_flag = th0->flags;
          sb->tcp_win = clib_net_to_host_u16 (th0->window);
          sb->src_port = clib_net_to_host_u16(th0->src_port); /* just to get entropy -> no need to clib_net_to_host_u16 */ 
          sb->d_n_flow = 0;
#endif /* LB_STATS */
          ...
          lb_hash_get (sticky_ht, hash0,
                       vip_index0, lb_time,
                       &available_index0, &asindex0, sb, vip_shm);
#else 
          lb_hash_get (sticky_ht, hash0,
                       vip_index0, lb_time,
          ...
          if (PREDICT_TRUE(asindex0 != 0))
          ...
          else if (PREDICT_TRUE(available_index0 != ~0))
            {
...
#ifdef LB_ALIAS /* alias method */
              f32 rand_num = ((float)rand())/RAND_MAX * vec_len(vip0->as_indexes);
              u32 bucket = (int) rand_num;
              alias_t weight = vip_shm->msg_in_cache->weights[vip0->as_indexes[bucket]];
              f32 odd = weight.odd;
              u32 alias =weight.alias;
              // u32 alias = lbm->ass[vip0->as_indexes[bucket]].score.alias;

              if ((rand_num - (f32) bucket) > odd)
              {
                asindex0 = vip0->as_indexes[alias];
              }
              else
              {
                asindex0 = vip0->as_indexes[bucket];
              }
              ...
              lb_hash_put (sticky_ht, hash0, asindex0,
                           vip_index0,
                           available_index0, lb_time, sb, vip_shm);
#else
              lb_hash_put (sticky_ht, hash0, asindex0,
                           vip_index0,
          ...
          shm_vip_if_update_msg (sb->time_now, vip_shm->msg_out_cache->ts) /* if it's time to update the shared memory */
          {
            shm_memcpy_frame_out(vip_shm, sb->time_now);
            /* assume that updating as score is always not faster than updating features */
            /* update AS scores if possible */
#ifdef LB_MSG_IN
            shm_memcpy_frame_in(vip_shm);
#endif // LB_MSG_IN
          }
#endif /* LB_STATS */
