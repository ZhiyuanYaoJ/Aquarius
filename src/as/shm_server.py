...
'''
This file runs on the server VMs
'''
...
def listen(conn, addr, shm):
    ...
    while True:
        ...
        if not data_recv:
            break
        conn.send(shm[42:66])  # [cpu:apache:memory:asid]
...
with open("/dev/shm/shm_vpp", "r+") as f:
    SHM = mmap.mmap(f.fileno(), 1048576, mmap.MAP_SHARED,
                    mmap.PROT_READ | mmap.PROT_WRITE, 0)

    while True:
        CONN, ADDR = SOCKET.accept()
        _thread.start_new_thread(listen, (CONN, ADDR, SHM))
SOCKET.close()
