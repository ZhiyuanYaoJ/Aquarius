...
'''
Shutdown VMs on remote server
'''
import argparse
import testbed_utils as tu

PARSER = argparse.ArgumentParser(description='Shutdown all servers.')
...
PARSER.add_argument('--episode', action='store',
                    default=1,
                    dest='episode',
                    help='Episode id')
...
_, TASK_DIR, _ = tu.init_task_info(
...
tu.NODES['clt'][0].fetch_result(TASK_DIR, ARGS.episode)
