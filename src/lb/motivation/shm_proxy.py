...
class Shm_Manager(object):
    ...
    def parse_frame_out(self, sid, frame):
        ...
        gt = None
        n_as_motiv = 88
        if self.gt: gt = np.zeros((n_as_motiv+1, len(GT)))
        threads = [Thread(target=self.process_as_feature, args=(sid, frame, asid, ts, feature_as, gt)) for asid in range(1, n_as_motiv+1)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
