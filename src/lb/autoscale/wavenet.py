...
FEATURE_SEQ_IDX = [i for i, f in enumerate(FEATURE_AS_ALL) if f in FEATURE_SEQ]

t0 = time.time()
preprocessor = load('preprocess.joblib')
wavenet = keras.models.load_model('wavenet.model')
print("Model loaded in {:.3f}s".format(time.time()-t0))
...
def make_action(delta, obs_cnt):
    ...
    if abs(delta) > np.ceil(actual_n_server/3):
        ...
        ts = time.time()
        obs_cnt[:actual_n_server] = 16
        subprocess_cmd(cmd)
    return obs_cnt, info, ts

if __name__ == '__main__':

    cool_down = 2 # cool down 2s
    n_step_x = 32
    obs = np.zeros((max_n_server, n_step_x, len(FEATURE_SEQ)))
    ...
    for step in range(max_steps):
        ...
        # auto-scale
        t_now = time.time()
        # feature_as w/ shape (n_servers, n_features)
        obs_step = preprocessor.transform(
            state[-2][max_n_server-actual_n_server+1:max_n_server+1, FEATURE_SEQ_IDX])[::-1]
        obs[:actual_n_server, :-1] = obs[:actual_n_server, 1:] # shift observation sequence
        # print("obs: {}".format(obs))
        obs[:actual_n_server, -1] = obs_step
        # print("updated obs: {}".format(obs))
        obs_cnt[:actual_n_server] += 1
        if t_now - t_last_action < cool_down or t_now - t0 < t_init or len([i for i in obs_cnt >= n_step_x if i]) <= np.ceil(actual_n_server/3):
            print(obs_cnt, len([i for i in obs_cnt>=n_step_x if i]))
            continue
        else:
            # print("obs_cnt: {}".format(obs_cnt))
            delta_lo_hi, info = get_cpu_n_lo_hi_delta(wavenet.predict(obs[obs_cnt>=n_step_x]))
            logger.info('[{:6.3f}s] {}\nactual_cpu = '.format(
                t_now - t0, info) + ' |'.join(
